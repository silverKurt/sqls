SELECT
    ...
    cast (CFATEORI as varchar) ||'-'|| cast (CFATDOCU as varchar)     as NotaFiscalUnica,
        ...
        CASE  WHEN CFATCLIE = 5000 then 'Consumidor Final' else 'Cliente' end as "Tipo Cliente",
...
SELECT
   ...
   CFATDFOR as formaPagamento,
    ...
    CFATPEDI      as Pedido,
        ...
        CFATCLIE      as CodClienteCont,
        CLI.CADANOME   as Cliente,
        CASE  WHEN CFATCLIE = 5000 then 'Consumidor Final' else 'Cliente' end as "Tipo Cliente",  replace(replace( coalesce(PAISNOME,'BRASIL')||','||EMP.CADAESTA||','||EMP.CADACIDA||','||EMP.CADABAIR||','||EMP.CADAENDE,'\"' , ''),'\'', '') as GeoLoja,
