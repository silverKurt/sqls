...
MEMBER [PeriodoFaturamento.(Trimestre)].[MÉDIA] AS
    (([PeriodoFaturamento].[Todos]) / [Measures].[MesescomRegistro])
...
NON EMPTY UNION([PeriodoFaturamento.(Trimestre)].[Todos].Children, [PeriodoFaturamento.(Trimestre)].[MÉDIA]) ON ROWS
FROM [MetaxFaturamentoNovo]
WHERE [lastFourTrimesters]