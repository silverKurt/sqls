...
SELECT 
 ...
 (cspecasa.quantidade*cspecasa.unitario-cspecasa.descunit) as VlrTotalOS,
 CASE WHEN csitens.tpordem IN ('AGD','ASI','ASR','CCM','IAC','IAD','IMN','IMU','IOF','IPC','RET')
 	  THEN 'ENCERRADA' ELSE 'FATURADA' END AS Status

FROM cscapas
 LEFT JOIN cspecasa ON (cspecasa.id_oscapa = cscapas.id_oscapa)
 LEFT JOIN csitens ON (csitens.id_oscapa = cspecasa.id_oscapa AND csitens.nroitem = cspecasa.nroitem)
 ...
 LEFT JOIN cntpitem tpi ON (cspecasa.tpitem = tpi.tpitem)

WHERE cscapas.dtabertura >= "2020-01-01"
  AND cscapas.id_oscapa NOT IN (SELECT csmotca.id_oscapa FROM csmotca)
  AND cscapas.dtfechament IS NOT NULL
  AND cspecasa.tpitem IN (1, 2, 3, 4, 7)
