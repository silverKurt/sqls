...
MEMBER [Measures].[ating] AS
    iif((isEmpty([Measures].[qtdlt]) OR isEmpty([Measures].[qtd_litros_AA2])), NULL, (([Measures].[qtdlt] / [Measures].[qtd_litros_AA2]) - 1.0)), $caption = "% de Atingimento", FORMAT_STRING = Iif((([Measures].[ating]) > (0.0)), "|0.00%|image=/resources/images/kpi/Arrow_Status_Asc3\.gif", "|0.00%|image=/resources/images/kpi/Arrow_Status_Asc1\.gif")
...
NON EMPTY Filter(NonEmptyCrossJoin([produto].[Todos].Children, [fabricante].[Todos].Children), ([Measures].[qtdlt] <> 0.0)) ON ROWS
