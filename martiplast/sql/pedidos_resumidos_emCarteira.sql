SELECT "Cliente"
        ...
        , SUM(CAST("VlrMercadoriaEmCarteira" AS REAL)) AS "VlrMercadoriaEmCarteira"
  FROM( 
  SELECT 
    ...
    , CASE WHEN ltrim(rtrim(p.cd_indice)) != '' and p.cd_indice IS NOT NULL THEN CAST(i.vl_total_item_l AS real)*(SELECT TOP 1 ii.Vl_cotacao_vend FROM GEINDICE ii WHERE ii.cd_indice = p.cd_indice ORDER BY dt_indice DESC) ELSE CAST(i.vl_total_item_l AS real) END AS "VlrMercadoriaEmCarteira"
