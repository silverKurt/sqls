WITH DADOS AS (
  ...
  SELECT DISTINCT Cd_material, Cd_Especif1 FROM ESMOVIME
    ...
    SELECT DISTINCT
       ...
       e.Elemento as 'cod_material',
       pl.Especif1 as 'cod_cor'

    FROM GEELEMEN e         (nolock)
    INNER JOIN ESPARPLA pl  (nolock)  ON (pl.Cd_material = e.Elemento)
    INNER JOIN ESESTOQU es  (nolock)  ON (es.Cd_material = e.Elemento AND es.Cd_especif1 = pl.Especif1)
    INNER JOIN ESES1 c      (nolock)  ON (es.Cd_especif1 = c.Cd_especif1)
    INNER JOIN ESDEMAND d   (nolock)  ON (es.Cd_material = d.Cd_material AND es.Cd_especif1 = d.Especif1)

    WHERE e.Cd_tg = '851'
    AND es.Cd_centro_armaz = '400'
    AND pl.Lote_minimo <> '0'
    AND d.Liberacao <> ''
...
SELECT DISTINCT ltrim(rtrim(a.cd_material)) + ' - ' + a.descricao + ' - ' + e.descricao  AS 'Material',
  ...
  WHERE a.Cd_grupo in ('50') AND
        ...
        a.Tipo         = 'A' AND
        a.cd_material IN (SELECT "cod_material" FROM tg851)
/*
